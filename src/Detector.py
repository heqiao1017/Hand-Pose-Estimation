...
tensorflow_path = base_path + "/phase1_models"
...
sys.path.append(tensorflow_path + "/research/object_detection/slim")
...
PATH_TO_CKPT = base_path + '/phase1_models/research/object_detection/hand_inference_graph/frozen_inference_graph.pb'
PATH_TO_LABELS = base_path + '/phase1_models/research/object_detection/training/hand_label_map.pbtxt' 
NUM_CLASSES = 1

#script from Tensorflow/object_detection tutorial
...
#predict images in a folder, from pos[0] to pos[1], call bounding_box_from_file inside
def bounding_box_from_folder(PATH_TO_TEST_IMAGES_DIR, padding, pos=None):
    ...
    if PATH_TO_TEST_IMAGES_DIR.endswith(('png', 'jpg')):
      ...
      file_list = sorted(os.listdir(PATH_TO_TEST_IMAGES_DIR))
      if pos != None:
...
#adjust bounding box to get a square box
def adjust_bound_box(xmin, ymin, xmax, ymax, padding, img_height, img_width):
  ...
  delta = height - width
  if delta > 0: #width is less than height
  ...
  #shift image if it is out of the image limit
  ...
  if bounding_box[0] < 0: #xmin
    ...
    bounding_box[0] -= delta
    bounding_box[2] -= delta
  #in case photo itself is already too large
...
#build 2d gaussian distribution map with a given center
...
#build haetmap for joints, will call make_2d_gaussian_map inside the function
